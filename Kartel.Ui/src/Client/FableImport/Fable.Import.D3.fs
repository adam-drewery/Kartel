...
module Selection =
    ...
    // type Vec3D(?versor: Vec3D) =
    //     member val x = 0 with get, set
    //     member val y = 0 with get, set
    //     member val z = 0 with get, set
    //     member this.cartesian (obj: obj) : Vec3D = jsNative
    //     member this.multiply (a:float, b:float) = jsNative
    //     member this.multiply (a:Vec3D, b:Vec3D) = jsNative
    //     member this.rotation (a:Vec3D) = jsNative
    //     member this.toIntArray() = [| this.x; this.y; this.z; |]
    //     member this.rotate () : Vec3D = jsNative
    //     member this.rotate (a: int[]) : Vec3D = jsNative
    //     member this.scale (a: float) : Vec3D = jsNative
    //     member this.translate (a: float[]) : Vec3D = jsNative
    //     member this.invert (a: float * float) : Vec3D = jsNative
    //     member this.delta (a: Vec3D * Vec3D) : Vec3D = jsNative
    //     // new() = Vec3D()
    //     new(versor) = Vec3D(versor)

    // [<AbstractClass>]
    // type Vec3D =
    //     abstract x: int with get, set
    //     abstract y: int with get, set
    //     abstract z: int with get, set
    //     abstract cartesian: Vec3D -> Vec3D
    //     abstract multiply: float * float -> Vec3D
    //     abstract multiply: Vec3D * Vec3D -> Vec3D
    //     abstract rotation: Vec3D -> Vec3D
    //     abstract rotate: unit -> Vec3D
    //     abstract rotate: (int[]) -> Vec3D
    //     abstract scale: (float) -> Vec3D
    //     abstract translate: (float[]) -> Vec3D
    //     abstract invert: (float * float) -> Vec3D
    //     abstract delta: (Vec3D * Vec3D) -> Vec3D
    //     // abstract toIntArray(): = [| this.x; this.y; this.z; |]
    //     // default member this.toIntArray
    //     new() = Vec3D()
    //     // new(versor) = Vec3D(versor)

    type GeoOrthographic =
        abstract precision: float -> float[]

    type Coordinate = float[]
        // member val x = x with get, set
        // member val y = y with get, set
        // member this.toIntArray() = [| this.x; this.y|]
        // new() = Coordinate()
    ...
    // type Countries() =
    //     member val features = Unchecked.defaultof<Feature[]>
    type Countries =
        abstract features: Feature[]

    type TopoJsonObjects =
        abstract ``land``: obj
        abstract countries: obj

    type TopoJsonTopology =
        abstract objects: TopoJsonObjects

    type TopoJsonFeature = obj

    type TopoJson =
        abstract feature: TopoJsonTopology -> obj -> Countries
        // abstract feature: TopoJsonTopology -> obj -> Countries
    ...
    //     abstract clearRect: float * float * float * float -> unit
    //     abstract fill: unit -> unit
    // type Versor() =
    //     member this.cartesian (obj: obj) : Versor = jsNative
    //     member this.multiply (a:float, b:float) = jsNative
    //     member this.rotation (a:Versor) = jsNative

    // type ContextObject(?obj: obj) =
    //     member val strokeStyle = "" with get, set
    //     member val fillStyle = "" with get, set
    //     member this.beginPath() = ()
    //     member this.stroke() = ()
    //     member this.fill() = ()
    //     member this.clearRect (u1: float, u2: float, width: float, height: float) =
    //         jsNative
    //     new(obj: obj) = ContextObject(obj)
    ...
    type GElement =
        abstract getContext: string -> ContextObject

    type EventListenerTypeString = string

    type Draggable =
        abstract on: (EventListenerTypeString * 'a) -> Draggable

    type Context =
        abstract context: CanvasRenderingContext2D -> obj -> ContextObject

    type Autorotate =
        abstract restart: (TimeSpan -> unit) -> float -> unit
    ...
    type [<AllowNullLiteral>] IExports =
        ...
        abstract geoOrthographic: unit -> GeoOrthographic
        abstract geoGraticule10: unit -> unit
        abstract geoPath: 'T -> Context
        abstract now: unit -> DateTime
        abstract timer: (TimeSpan -> unit) -> obj

        // [<ImportDefault("./../../../node_modules/d3-fetch/src/json.js")>]
        // abstract json: (string * 'a) -> Promise<obj[]>
        static member tsv (url: string) (obj: obj) : obj[] = import "json" "./../../../node_modules/d3-fetch/src/index.js"
        // abstract versor: Vec3D
        abstract drag: unit -> 'a
        // abstract tsv: (UrlString * 'a) -> obj[]
        // abstract drag: unit -> Selection<'a,'b,Browser.Types.HTMLElement,obj option> -> ResizeArray<obj option> -> Draggable

        // abstract versor: cartesian: obj -> Vec3D
        // abstract versor: unit -> Vec3D
        // abstract topojson: feature: TopoJsonTopology * obj[] -> 'Feature
